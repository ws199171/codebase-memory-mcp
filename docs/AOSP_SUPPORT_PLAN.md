...
- Deferred verification:
  ...
  - `B3` target, arch, multilib, product-variable, legacy Soong-config, and static
    `select(...)` dependency variants; multi-branch aggregation, conditional
  ...
  - `B9` module definition/source/tool file resolution, normalized filegroup paths,
    ...
    required before verification completion.
  - `B10` bounded CLI/MCP module details, resolved and unresolved dependency edges,
    structured variant/defaults/visibility/reference provenance, file declaration
    link states, concrete Make/Bazel coverage gaps, deterministic ordering,
  ...
  - `P3` literal ServiceManager registration, lookup, check, and wait operations;
    shared cross-repository service identities; enclosing client/server callers;
    unique implementation and AIDL interface links; missing, ambiguous, dynamic
